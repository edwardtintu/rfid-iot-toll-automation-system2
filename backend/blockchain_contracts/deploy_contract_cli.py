...
def deploy_contract_with_cli_key(private_key):
    # Connect to Ganache
    ...
    # Try different possible attributes for the raw transaction
    raw_tx = None
    for attr in ['raw_transaction', 'rawTransaction', 'raw']:  # raw_transaction is the correct attribute name
    ...
    if raw_tx is None:
        ...
        print(f"Available attributes: {[attr for attr in dir(signed_txn) if not attr.startswith('_')]}")
        return None, None
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python deploy_contract_cli.py <private_key>")
        print("Example: python deploy_contract_cli.py 0x...")
        return 1
    
    private_key = sys.argv[1]
    contract_address, abi = deploy_contract_with_cli_key(private_key)
